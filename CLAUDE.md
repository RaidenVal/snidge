...
- [x] LG 电脑回归测：这台机器建 D3D 设备本身要 ~594ms（跟改动前单独测出的 ~500ms 量级一致），但设备复用后单次取色稳定在 ~261-280ms、总耗时 ~312-334ms——比这台机器改动前的 ~800-1000ms 快了近 3 倍。颜色正确（用户特意在取色间移动鼠标/切换画面内容验证过不是陈旧帧）、崩溃自动 fallback 正常、app 退出无孤儿进程残留。150ms 冲刷窗口在这台机器上够用。
- [ ] 多显示器手测（当前没有测试条件，暂缓）
